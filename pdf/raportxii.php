...
$pdf->Cell(3);
$pdf->Cell(10,$f,$no++,'LRB',0, 'C');
$pdf->Cell(50,$f,'Sejarah Indonesia', 'LRB',0, 'L');
...
$pdf->Cell($width_cell[0],6,'B. Praktek Kerja Industri',0,1);
$width_cell=array(10,115,60);
$pdf->Cell(3);
    $pdf->Cell($width_cell[0],10,'No',1,0,'C');
    $pdf->Cell($width_cell[1],10,'Nama Bengkel ',1,0,'C');
    $pdf->Cell($width_cell[2],10,'Nilai',1,1,'C');
    $pdf->SetFont('Arial','',8);
$h=8;
...
$pdf->Cell($width_cell[0],6,'C. Ekstrakurikuler',0,1);
...
$pdf->Ln(10);
$pdf->Cell(3);
$pdf->Cell($width_cell[0],6,'D. Ketidakhadiran',0,1);
$pdf->SetFont('Arial','',8);
...
$width_cell=array(185);
$pdf->Cell(3);
        $pdf->Cell($width_cell[0],$h,'Berdasarkan hasil yang dicapai pada Semester 5 dan 6, peserta didik ditetapkan : LULUS',1,1,'L');
    $pdf->SetFont('Arial','',8);
...
$width_cell=array(110,65);
 ...
 $pdf->Output('D','Raport '. $row['kelas'].' '. $row['jurusan'].' '. $row['pemkelas'].' '.$row['nama'].' '.$leger['semester'].'pdf');
}?>